...
val repositoryModule = Kodein.Module {
    bind<ArtistRepository>() with singleton { ArtistRepositoryImp(listOf(CloudArtistDataSet(instance(Qualifiers.LanguageSelection), instance()))) }
    bind<AlbumRepository>() with singleton { AlbumRepositoryImpl(listOf(CloudAlbumDataSet(instance()))) }
}

private val KodeinAwareBase.appContext get() = instance<Context>(Qualifiers.ApplicationContext)
